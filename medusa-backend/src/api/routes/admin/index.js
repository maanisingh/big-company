...
router.get('/categories', authenticateJWT, requireAdmin, wrapHandler(async (req: any, res) => {
  ...
  try {
    ...
    const countResult = await db.query(`
      SELECT COUNT(*) as total FROM bigcompany.product_categories c ${whereClause}
    `, params);
...
router.post('/categories', authenticateJWT, requireAdmin, wrapHandler(async (req: any, res) => {
  ...
  try {
    ...
    const existingCategory = await db.query(
      'SELECT id FROM bigcompany.product_categories WHERE LOWER(name) = LOWER($1)',
      [name.trim()]
    );
    ...
    `, [adminId, result.rows[0].id, JSON.stringify({ name, code, description })]);
...
router.put('/categories/:id', authenticateJWT, requireAdmin, wrapHandler(async (req: any, res) => {
  ...
  try {
    ...
    const existingCategory = await db.query(
      'SELECT * FROM bigcompany.product_categories WHERE id = $1',
      [id]
    );
    ...
    const duplicateCategory = await db.query(
      'SELECT id FROM bigcompany.product_categories WHERE LOWER(name) = LOWER($1) AND id != $2',
      [name.trim(), id]
    );
