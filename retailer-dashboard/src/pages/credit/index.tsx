...
export const CreditOrderList = () => {
  ...
  const handleRecordRepayment = async () => {
    ...
    try {
      ...
      message.success(`Repayment of ${repaymentAmount.toLocaleString()} RWF recorded successfully`);
      ...
      render: (_: any, record: CreditOrder) => (
        <Space>
          <Button
            icon={<EyeOutlined />}
            size="small"
            onClick={() => navigate(`/credit-orders/show/${record.id}`)}
          >
            View
          </Button>
        ...
        <Col xs={24} sm={12} lg={6}>
          <Card>
            <Statistic
              ...
              formatter={(value) => value?.toLocaleString()}
            />
            <Text type="secondary">{stats?.total_approved || 0} active</Text>
          </Card>
        </Col>
        <Col xs={24} sm={12} lg={6}>
          <Card>
...
export const CreditOrderShow = () => {
  ...
  const fetchOrder = async () => {
    ...
    try {
      const data = await creditApi.getOrder(id!);
      setOrder(data.order || data);
    } catch (err: any) {
      setError(err.response?.data?.error || 'Failed to load credit order');
  ...
  const handleApprove = async () => {
    ...
    try {
      await creditApi.approveCredit(order.id, approveTerms);
      message.success('Credit order approved successfully');
  ...
  const handleReject = async () => {
    ...
    try {
      await creditApi.rejectCredit(order.id, rejectReason);
      message.success('Credit order rejected');
  ...
  const handleRecordRepayment = async () => {
    ...
    try {
      await creditApi.recordRepayment(order.id, repaymentAmount, repaymentMethod);
    ...
    return (
      <div style={{ padding: '24px' }}>
        <Alert
          ...
          showIcon
        />
        <Button style={{ marginTop: '16px' }} onClick={() => navigate('/credit-orders')}>
  ...
  return (
    <div style={{ padding: '24px' }}>
      <Button
        ...
        onClick={() => navigate('/credit-orders')}
        style={{ marginBottom: '16px' }}
      >
        Back to Credit Orders
      </Button>
      ...
      <Row gutter={24}>
        <Col xs={24} lg={16}>
          ...
          <Card title="Order Details" style={{ marginBottom: '16px' }}>
            <Row gutter={24}>
              <Col span={12}>
                <Descriptions column={1} size="small" bordered>
                  <Descriptions.Item label="Customer">
                    <div>
                      <div><UserOutlined /> {order.customer_name}</div>
                      <div><PhoneOutlined /> {order.customer_phone}</div>
                    </div>
                  </Descriptions.Item>
                  <Descriptions.Item label="Credit Score">
                    <Progress
                      percent={order.credit_score}
                      strokeColor={getCreditScoreColor(order.credit_score)}
                      format={(p) => `${p} - ${getCreditScoreLabel(p || 0)}`}
                    />
                  </Descriptions.Item>
                  <Descriptions.Item label="Credit Limit">
                    {order.credit_limit?.toLocaleString()} RWF
                  </Descriptions.Item>
                </Descriptions>
              </Col>
              <Col span={12}>
                <Descriptions column={1} size="small" bordered>
                  ...
                  <Descriptions.Item label="Total Amount">
                    <Text strong style={{ fontSize: '16px' }}>{order.total_amount?.toLocaleString()} RWF</Text>
                  </Descriptions.Item>
                  <Descriptions.Item label="Amount Paid">
                    <Text type="success">{order.amount_paid?.toLocaleString()} RWF</Text>
                  </Descriptions.Item>
                  <Descriptions.Item label="Amount Pending">
                    <Text type="warning">{order.amount_pending?.toLocaleString()} RWF</Text>
                  </Descriptions.Item>
            ...
            {order.rejected_reason && (
              <Alert
                ...
                description={order.rejected_reason}
                type="error"
                style={{ marginTop: '16px' }}
              />
            )}
              ...
              summary={() => (
                <Table.Summary.Row>
                  <Table.Summary.Cell index={0} colSpan={3}>
                    <Text strong>Total Credit Amount</Text>
                  </Table.Summary.Cell>
                  <Table.Summary.Cell index={1}>
                    <Text strong style={{ color: '#0ea5e9' }}>{order.total_amount?.toLocaleString()} RWF</Text>
                  </Table.Summary.Cell>
                </Table.Summary.Row>
              )}
            />
          ...
          {/* Repayment History */}
          {order.repayments && order.repayments.length > 0 && (
            <Card title={<><HistoryOutlined /> Repayment History</>} style={{ marginBottom: '16px' }}>
              <Timeline>
                {order.repayments.map((repayment) => (
        ...
        <Col xs={24} lg={8}>
          ...
          <Card title="Actions" style={{ marginBottom: '16px' }}>
            <Space direction="vertical" style={{ width: '100%' }}>
              ...
              {order.status === 'approved' && order.amount_pending > 0 && (
                <Button
                  type="primary"
                  icon={<DollarOutlined />}
                  block
                  onClick={() => {
                    setRepaymentAmount(order.amount_pending);
              ...
              {order.status === 'repaid' && (
                <Alert
                  ...
                  description="This credit order has been fully repaid."
                  type="success"
                  showIcon
                />
              )}
              {order.status === 'rejected' && (
                <Alert
                  message="Rejected"
                  description="This credit order was rejected."
                  type="error"
                  showIcon
                />
              )}
          ...
          {/* Payment Progress */}
          {(order.status === 'approved' || order.status === 'repaid') && (
            <Card title="Payment Progress" style={{ marginBottom: '16px' }}>
              <Progress
                percent={Math.round((order.amount_paid / order.total_amount) * 100)}
                strokeColor={order.amount_pending === 0 ? '#52c41a' : '#1890ff'}
                status={order.amount_pending === 0 ? 'success' : 'active'}
              />
              <div style={{ marginTop: '16px' }}>
                <Row justify="space-between">
                  <Col><Text type="secondary">Paid:</Text></Col>
                  <Col><Text type="success" strong>{order.amount_paid?.toLocaleString()} RWF</Text></Col>
                </Row>
                <Row justify="space-between">
                  <Col><Text type="secondary">Remaining:</Text></Col>
                  <Col><Text type="warning" strong>{order.amount_pending?.toLocaleString()} RWF</Text></Col>
                </Row>
              </div>
            </Card>
          )}
          ...
          <Card title="Order Timeline">
            <Timeline>
              <Timeline.Item color="blue">
                <div><Text strong>Order Created</Text></div>
                <Text type="secondary">{new Date(order.created_at).toLocaleString()}</Text>
              </Timeline.Item>
              {order.approved_date && (
                <Timeline.Item color="green">
                  <div><Text strong>Order Approved</Text></div>
                  <Text type="secondary">{new Date(order.approved_date).toLocaleString()}</Text>
                </Timeline.Item>
              )}
              {order.status === 'repaid' && (
                <Timeline.Item color="green">
                  <div><Text strong>Fully Repaid</Text></div>
                  <Text type="secondary">{new Date(order.updated_at).toLocaleString()}</Text>
                </Timeline.Item>
              )}
              {order.status === 'rejected' && (
                <Timeline.Item color="red">
                  <div><Text strong>Order Rejected</Text></div>
                  <Text type="secondary">{new Date(order.updated_at).toLocaleString()}</Text>
                </Timeline.Item>
              )}
            </Timeline>
      ...
      >
        <Alert
          message={`Approve ${order.total_amount?.toLocaleString()} RWF credit for ${order.customer_name}?`}
          description={
            <div>
              <p>Credit Score: <strong>{order.credit_score}</strong> ({getCreditScoreLabel(order.credit_score)})</p>
              <p>Credit Limit: <strong>{order.credit_limit?.toLocaleString()} RWF</strong></p>
            </div>
          }
          type="info"
          style={{ marginBottom: '16px' }}
      ...
      >
        <Alert
          message={`Reject ${order.total_amount?.toLocaleString()} RWF credit request?`}
          type="warning"
          style={{ marginBottom: '16px' }}
        />
        <Form layout="vertical">
      ...
      >
        <Descriptions column={1} size="small" style={{ marginBottom: '16px' }}>
          ...
          <Descriptions.Item label="Outstanding Amount">
            <Text type="warning" strong>{order.amount_pending?.toLocaleString()} RWF</Text>
          </Descriptions.Item>
        </Descriptions>
        ...
        {repaymentAmount >= order.amount_pending && (
          <Alert
            message="Full Payment"
            description="This payment will fully settle the credit order."
            type="success"
